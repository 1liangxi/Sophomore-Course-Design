...
<html>
    ...
    <div id="table">
    <div width="100%">
            <form action="../room/Room_searchDoctor.action" method="post">
                <input id="search" type="text" placeholder="输入科室编号/科室名" name="search">
                <input type="submit" id="search1" value="查找">
            </form>
        </div>
    <form id="myform" action="<%=path %>/room/Room_delectAll.action">
        <table class="table table-bordered table-hover" width="100%">
            ...
            <%
            	...
            	List<Room> list = (List)session.getAttribute("Room_List");
            	size = list.size();
            	if(pagenum==1)
            	{
            		j = 0;
            ...
            <tr class="list">
            	<td><input name="checkbox" value="<%=p.getRno() %>" class="check-only check" type="checkbox"></td>
                <td><%=p.getRno() %></td>
                <td><a href="<%=path %>/room/Room_queryByRno.action?rno=<%=p.getRno() %>"><%=p.getRname() %></a></td>
                <td><%=p.getRphone() %></td>
                <td><%=p.getDid() %></td>
                <td><%=p.getDname() %></td>
                <td><%=p.getRtime() %></td>
                
                <td><a href="<%=path %>/room/Room_delete.action?did=<%=p.getRno() %>">删除</a></td>
        ...
        <div style="margin:0px auto;width:420px;">
            <form action="../room/Room_query.action">
            <span><a href = "../room/Room_query.action" >首页</a></span>
            <span><a href = "../room/Room_query_success.jsp?pagenum=<%=pagenum-1 %>" >上一页</a></span>
            <span>
		          <input id="pageselect" type="text" name="pagenum" value="<%=pagenum %>" style="width:30px;height:20px;text-align:center;">
		      </span>
            <span><a  href = "../room/Room_query_success.jsp?&pagenum=<%=1+pagenum %>">下一页</a></span>
            <span><a href = "../room/Room_query_success.jsp?&pagenum=<%=size/max+1 %>" >尾页</a></span>
