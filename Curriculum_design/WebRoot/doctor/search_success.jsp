...
<html>
    ...
    <body>
    <form action="../doctor/Doctor_searchDoctor.action" method="post">
    ...
    <form id="myform" action="<%=path %>/doctor/Doctor_delectAll.action">
        <table class="table table-bordered table-hover" width="100%">
            ...
            <%
            	...
            	List<Doctor> list = (List)session.getAttribute("Doctor_List");
            	size = list.size();
            	if(pagenum==1)
            	{
            		j = 0;
            ...
            <tr class="list">
            	<td><input name="checkbox" value="<%=p.getDid() %>" class="check-only check" type="checkbox"></td>
                <td><%=p.getDid() %></td>
                <td><a href="<%=path %>/doctor/Doctor_queryByDid.action?did=<%=p.getDid() %>"><%=p.getDname() %></a></td>
                <td><%=p.getDidcard() %></td>
                <td><%=p.getDsex() %></td>
                <td><%=p.getDroom() %></td>
                <td><%=p.getDjod() %></td>
                <td><%=p.getPhone() %></td>
                <td><a href="<%=path %>/doctor/Doctor_delete.action?did=<%=p.getDid() %>">删除</a></td>
        ...
        <div style="margin:0px auto;width:420px;">
            <form action="../doctor/Doctor_query.action">
            <span><a href = "../doctor/Doctor_query.action" >首页</a></span>
            <span><a href = "../doctor/Doctor_query_success.jsp?pagenum=<%=pagenum-1 %>" >上一页</a></span>
            <span>
		          <input id="pageselect" type="text" name="pagenum" value="<%=pagenum %>" style="width:30px;height:20px;text-align:center;">
		      </span>
            <span><a  href = "../doctor/Doctor_query_success.jsp?&pagenum=<%=1+pagenum %>">下一页</a></span>
            <span><a href = "../doctor/Doctor_query_success.jsp?&pagenum=<%=size/max+1 %>" >尾页</a></span>
